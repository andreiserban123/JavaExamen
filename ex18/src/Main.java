...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        try (BufferedReader bf = new BufferedReader(new FileReader("Date\\carti.txt"))) {
            carti = bf.lines().map(line -> new Carte(line.split("\t")[0], line.split("\t")[1], line.split("\t")[2], Integer.parseInt(line.split("\t")[3]))).toList();
        }

        try (var conn = DriverManager.getConnection("jdbc:sqlite:Date\\biblioteca.db"); var stm = conn.createStatement(); var resultSet = stm.executeQuery("SELECT * from imprumuturi");) {
            ...
            stm.executeUpdate("Delete from imprumuturi");
        }
        ...
        try (BufferedWriter bw = new BufferedWriter(new FileWriter("Date\\cititori.txt"))) {
            ...
            biblioteca.stream().collect(Collectors.groupingBy(i -> i.numeStudent, Collectors.summingInt(c -> c.nrZile))).entrySet().stream().sorted(Map.Entry.<String, Integer>comparingByValue().reversed()).forEach(e -> {
                try {
        ...
        try (var conn = DriverManager.getConnection("jdbc:sqlite:Date\\biblioteca.db"); var pstm = conn.prepareStatement("INSERT INTO imprumuturi values (?,?,?)");) {
            biblioteca.forEach(b -> {
                try {
                    pstm.setString(1, b.numeStudent);
                    pstm.setString(2, b.cota);
                    pstm.setInt(3, b.nrZile);
                    pstm.executeUpdate();
